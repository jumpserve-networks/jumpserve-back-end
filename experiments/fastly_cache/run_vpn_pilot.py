#!/usr/bin/env python3
"""Run a controlled Fastly cache pilot through sequential Mullvad exits."""
...
TARGET_ORIGIN = (
    "http://jumpserve-cache-study-20260827-01.global.ssl.fastly.net."
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Switch through Mullvad exits and run a fresh randomized Fastly "
            "hot/cold cache trial at each exit."
        )
    )
    parser.add_argument("--output-dir", required=True, type=Path)
    parser.add_argument("--objects-per-class", type=int, default=10)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        for index, location in enumerate(locations, start=1):
            run_slug = f"fastly-vpn-{batch_id.lower()}-{location.city}"
            ...
            exit_path = prefix.with_suffix(".mullvad-exit.json")
            ...
            try:
                ...
                exit_path.write_text(exit_result.stdout, encoding="utf-8")
    ...
    summary = {
        "batch_id": batch_id,
        "fastly_service_id": "i9hnKYTkxIksRHg8fK0dx7",
